...
from ..plugins.backup_manager import LocalBackupManagerModule

backup_manager = LocalBackupManagerModule()

@otp_bp.route('/calls/otp/enroll', methods=['POST'])
def enroll_user():
    data = request.get_json()
    user_id = data.get('user_id', 'user001')
    # Optionally accept passphrase or recovery info
    passphrase = data.get('passphrase')
    recovery_info = data.get('recovery_info', {})
    codes = backup_manager.load_backup_codes(user_id)
    if not codes:
        codes = generate_backup_codes()
        # Store with Fernet encryption and recovery info
        backup_manager.store_backup_codes(user_id, codes, password=None, recovery_info=recovery_info)
